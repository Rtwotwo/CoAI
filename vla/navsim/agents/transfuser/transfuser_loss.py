...
Author: Redal
Date: 2026-01-28
Todo: Create Transfuser Loss for training the Transfuser model
...
def transfuser_loss(targets: Dict[str, torch.Tensor],
    ...
    targets: dictionary of name tensor pairings
    predictions: dictionary of name tensor pairings
    config: global TransFuser config
...
def _agent_loss(targets: Dict[str, torch.Tensor],
                predictions: Dict[str, torch.Tensor],
                config: TransfuserConfig):
    """Helper function calculating agent loss of Transfuser
    targets: dictionary of name tensor pairings
    predictions: dictionary of name tensor pairings
    config: global TransFuser config
    return: agent class loss, agent box loss"""
    ...
    # If latent space mode is enabled, filter the valid ground truth labels
    # based on the angle threshold relative to the ego vehicle
    if config.latent:
    ...
    # get batch size, number og predicted instances and number of gt instances
    # add 1 if num_gt_instances is zero to aviod empty cost matrix
    ...
    # compute cost matrix by ce and l1 cost matrix
    ...
    cost = cost.cpu()
    # Execute the Hungarian algorithm to obtain the optimal matching indices
    ...
    # get pred states and gt states for matched indices
    ...
    gt_valid_idx = torch.cat([t[i] for t, (_, i) in zip(gt_valid, indices)], dim=0)
    # compute l1 loss and ce loss by valid samples
    l1_loss = F.l1_loss(pred_states_idx, gt_states_idx, reduction='none')
    l1_loss = l1_loss.sum(dim=-1) * gt_valid_idx
...
def _get_ce_cost(gt_valid: torch.Tensor,
    ...
    """Helper function calculating cross entropy cost for cost matrix
    gt_valid: tensor of binary gt labels
    pred_logits: tensor of predicted logits of neural network
    return a tensor of cost matrix"""
    gt_valid_expanded = gt_valid[:, :, None].detach().float() # [batch_size, num_gt_instances, 1]
    pred_logits_expanded = pred_logits[:, None, :].detach() # [batch_size, 1, num_pred_instances]
    # Calculate the cross-entropy loss as the cost matrix
    ...
    ce_cost = (1 - gt_valid_expanded) * pred_logits_expanded + helper_term # # [batch_size, num_gt_instances, num_pred_instances]
    ce_cost = ce_cost.permute(0, 2, 1) # [batch_size, num_pred_instances, num_gt_instances]
    return ce_cost


@torch.no_grad()
def _get_l1_cost(gt_states: torch.Tensor,
                 pred_states: torch.Tensor,
                 gt_valid: torch.Tensor
                 )->torch.Tensor:
    """Helper function calculating l1 cost for cost matrix
    gt_states: tensor of gt states
    pred_states: tensor of predicted states
    return a tensor of cost matrix"""
    gt_states_expanded = gt_states[:, :, None, :2].detach() # [batch_size, num_gt_instances, 1, 2]
    pred_states_expanded = pred_states[:, None, :, :2].detach() # [batch_size, 1, num_pred_instances, 2]
    # compute l1 loss for l1 cost matrix
...
def _get_src_permutation_idx(indices):
    """Helper function for align indices after matching
    indices: matched indices; return permuted indices"""
