...
class ScaleNorm(nn.Module):
    ...
    def forward(self, x:torch.Tensor)->torch.Tensor:
        # 计算L2范数: sqrt(sum(x^2, dim=-1, keepdim=True))
        norm = torch.norm(x, dim=-1, keepdim=True)
        # 对范数进行clamp处理,防止数值过小导致梯度爆炸
        norm = torch.clamp(norm, min=self.clamp_min)
        # 进行归一化: x / norm
        x_norm = x / norm
        # 应用可学习的缩放参数
...
class ScaleNormFp32(ScaleNorm):
    """使用float16/bfloat16进行低精度计算时,归一化的L2范数计算
    可能会因精度不足导致数值不稳定"""
    def forward(self, x:torch.Tensor)->torch.Tensor:
        orig_type = x.dtype
        if x.dtype in [torch.float16, torch.bfloat16]:
...
class GroupNorm(nn.Module):
    """GroupNorm将通道分成若干组,每组内部进行归一化计算
    GN(x) = gamma * (x - E(x)) / sqrt(Var(x) + eps) + bias
    相比BatchNorm,GroupNorm不依赖batch size,适合小batch训练
    num_groups: 分组数量; num_channels: 输入通道数
    eps: 防止除零的小常数; affine: 是否进行仿射变换"""
    def __init__(self, num_groups: int,
        ...
        self.eps = eps
        self.affine = affine
        # 检查num_channels是否能被num_groups整除
        if num_channels % num_groups != 0:
            raise ValueError(f"num_channels({num_channels})必须能被num_groups({num_groups})整除")
        # 每组的通道数
    ...
    def forward(self, x:torch.Tensor)->torch.Tensor:
        # x.shape形状[N, C, H, W]或[N, L, C]
        if x.dim() < 2:
            raise ValueError(f'[WARNING] 输入的维度需要大于2D,实际{x.dim()}D!')
        # 保存原始形状用于恢复
        orig_shape = x.shape
        # 将输入reshape为[N, G, C//G, ...]便于分组计算
        N, C = x.shape[:2]
        # 将通道维度拆分为(num_groups, channels_per_group)
        x = x.view(N, self.num_groups, self.channels_per_group, *orig_shape[2:])
        # 计算均值和方差,在每组内的所有通道和空间位置上计算
        # 维度: [N, G, C//G, H, W] -> 在dim=(2,3,4,...)上计算
...
class GroupNormFp32(GroupNorm):
    """使用float16/bfloat16进行低精度计算时,组归一化的均值,
    方差计算可能会因精度不足导致数值不稳定"""
    def forward(self, x:torch.Tensor)->torch.Tensor:
        orig_type = x.dtype
        if x.dtype in [torch.float16, torch.bfloat16]:
